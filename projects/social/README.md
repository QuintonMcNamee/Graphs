...
You would need to call `add_friendship()` 500 times because the for loop that calls `add_friendship()` looks like this:

for i in range(num_users * avg_friendships // 2):
            friendship = possible_friendships[i]
            self.add_friendship(friendship[0], friendship[1])

As you can see the range for the for loop is "(num_users * avg_friendships // 2)" and if you insert 100 for num_users and 10 for avg_friendships you get 500 interations of the loop.
